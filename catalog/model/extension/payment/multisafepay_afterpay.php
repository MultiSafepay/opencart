 ...
 */
class ModelExtensionPaymentMultiSafePayAfterpay extends Model
{
    ...
    public function getMethod($address, $total)
    {
        ...
        if ($status) {
            ...
            if ($this->config->get('payment_multisafepay_use_payment_logo') == true ) {
                $title = '<img  height=32 width=auto  src="./image/multisafepay/afterpay.svg" alt="afterpay" title="afterpay" style="vertical-align: middle;" />';
                $terms = $this->language->get('text_title_afterpay');
            }else{
                $title = $this->language->get('text_title_afterpay');
                $terms = '';
            }
