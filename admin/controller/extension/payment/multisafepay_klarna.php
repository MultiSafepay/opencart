    ...
    public function index()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == "POST")){
            foreach($data['stores'] as $store) {
                $this->model_setting_setting->editSetting('payment_multisafepay_klarna', $this->request->post['stores'][$store['store_id']], $store['store_id']);
            }
        ...
        $fields = array (
            'payment_multisafepay_klarna_status',
            'payment_multisafepay_klarna_min_amount',
            'payment_multisafepay_klarna_max_amount',
            'payment_multisafepay_klarna_geo_zone_id',
            'payment_multisafepay_klarna_sort_order');

        foreach ($data['stores'] as $store) {
            $store_id = $store['store_id'];
            $config = $this->model_setting_setting->getSetting('payment_multisafepay_klarna', $store_id);
