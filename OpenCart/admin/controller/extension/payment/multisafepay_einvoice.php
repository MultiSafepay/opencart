...
class ControllerExtensionPaymentMultiSafePayeinvoice extends Controller
{
    ...
    public function index()
    {
        ...
        $this->load->language('extension/payment/multisafepay_einvoice');
        ...
        if (($this->request->server['REQUEST_METHOD'] == "POST")){
            foreach($data['stores'] as $store) {
                $this->model_setting_setting->editSetting('payment_multisafepay_einvoice', $this->request->post['stores'][$store['store_id']], $store['store_id']);
            }
        ...
        $fields = array (
            'payment_multisafepay_einvoice_status',
            'payment_multisafepay_einvoice_min_amount',
            'payment_multisafepay_einvoice_max_amount',
            'payment_multisafepay_einvoice_geo_zone_id',
            'payment_multisafepay_einvoice_sort_order');

        foreach ($data['stores'] as $store) {
            $store_id = $store['store_id'];
            $config = $this->model_setting_setting->getSetting('payment_multisafepay_einvoice', $store_id);
